...
namespace AlliancesPlugin.Alliances
{
    ...
    public static class MyProductionPatch
    {
        ...
        public static float GetAssemblerSpeedMultiplier(long PlayerId)
        {
     ...
     MyFixedPoint blueprintAmount, MyRefinery __instance)
        {
            ...
            }
            {
                ...
                if (blueprintAmount == (MyFixedPoint)0)
                    return false;
                if (refin.GetOwnerFactionTag().Length <= 0) return true;
                ...
                if (upgrades.TryGetValue(alliance.RefineryUpgradeLevel, out RefineryUpgrade upgrade))
                {
                    if (TimeChecks.TryGetValue(refin.EntityId, out DateTime time))
                    {
                        if (DateTime.Now >= time)
                        {
                            ...
                            {
                                foreach (var ter in from ter in AlliancePlugin.Territories.Values where ter.Alliance != Guid.Empty && ter.Alliance == alliance.AllianceId let distance = Vector3.Distance(refin.CubeGrid.PositionComp.GetPosition(),
                                             new Vector3(ter.x, ter.y, ter.z)) where distance <= ter.Radius select ter)
                                {
                                    IsInsideTerritory.Remove(refin.EntityId);
                                    IsInsideTerritory.Add(refin.EntityId, true);
                                    InsideHere.Remove(refin.EntityId);
